...
def parse_args():
    ...
    parser.add_argument('--render', type=bool, default=False, help='is need render result')
...
def dump_json(theta_all):
    # dump to directorys 
    import shutil
    dir_path = 'output/smpl' 
    if os.path.exists(dir_path):
        shutil.rmtree(dir_path)
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    idx = 0
    for arr in theta_all:
        parts = arr
        item = {
            'id': 0,
            'Rh': [parts[:3].tolist()],
            'Th': [[0, 0, 0]],
            'poses': [parts[3:72].tolist()],
            'shapes': [parts[72:].tolist()],
        }
        result = []
        result.append(item)
        dst_path = dir_path +'/'+("{:06d}".format(idx))+".json"
        idx = idx + 1
...
with torch.no_grad():
    for batch_input in tqdm(test_loader):
        ...
        output_theta = output[0]['theta'][:, :, :82].reshape(-1, 82)
        # 左右半身翻转，进行数据增强，降低抖动
        ...
        theta_all.append(output_theta.cpu().numpy())
...
theta_all = np.concatenate(theta_all) # (305 ,82)
